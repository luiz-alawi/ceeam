...
import { Loader2, KeyRound, ShieldCheck, User, Copy, Check } from 'lucide-react';
          ...
          filtered.map((u) => (
            <div key={u.id} className="bg-white rounded-2xl border border-[var(--line)] p-4 flex items-center justify-between gap-4">
              ...
              <button onClick={() => handleReset(u)} disabled={busyId === u.id}
                className="flex items-center gap-1.5 px-3 py-1.5 border border-[var(--line)] text-[var(--ink)] rounded-lg text-[12px] font-medium hover:bg-[var(--paper)] transition-colors disabled:opacity-60 shrink-0">
